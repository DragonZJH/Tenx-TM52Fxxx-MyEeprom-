	...
	switch ( flag )
	{
		...
		case 'A'://测试读取每个区域写入次数的,
			...
			for ( t=0; t<EepromMaxArea; t++ )
			{
             ...
             area==0xff?area=0:nop;
             
			//已知区域直接根据所在区域和地址读取相应的数据
			MyEeprom.EepromTestWriteCount = ( unsigned long ) EepromReadWithArea ( CS_CountAddr_L,area );
			MyEeprom.EepromTestWriteCount|= ( ( unsigned long ) EepromReadWithArea ( CS_CountAddr_H,area ) <<8 );
			MyEeprom.EepromTestWriteCount|= ( ( unsigned long ) EepromReadWithArea ( SetStableDukAddr_L,area ) <<16 );
			MyEeprom.EepromTestWriteCount|= ( ( unsigned long ) EepromReadWithArea ( SetStableDukAddr_H,area ) <<24 );
#endif
			break;
		case 'B':
			//from:外部给定的变量地址 数据大小即为写入的速度
			//用测试EEprom擦写极限或擦写寿命
			if ( * ( ( unsigned int* ) from ) >=40 ) //=500为5秒写一次 ，10=100ms 40=400ms
			{
				* ( ( unsigned int* ) from ) =0;
				//更新所有数据
				MyEepromWrite ( NULL,NULL,TRUE );
...
/*****************************************************************************
 ...
 Description  : 用户接口,程序自动选择的当前区域中保存用户的所有的数据
                注意:
                    1 此接口必须写入所有需要保存的数据
                    2 此接口处单个字节写调用EepromWriteByte()函数
